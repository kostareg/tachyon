...
std::unique_ptr<IRNode> LoweringVisitor::visit(VariableRefNode& node) {
    return nullptr;
}
...
std::unique_ptr<IRNode> LoweringVisitor::visit(SequenceNode& node) {
    std::for_each(node.body.begin(), node.body.end(), [this](std::unique_ptr<IRNode>& elem){
        elem->accept(*this);
    });
    return nullptr;
}
