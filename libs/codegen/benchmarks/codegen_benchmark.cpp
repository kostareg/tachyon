...
class CodegenDataFixture : public benchmark::Fixture {
  ...
  private:
    ...
    parser::Op strToOp(std::string str) {
        if (str == "&&") return parser::Op::And;
        else if (str == "||") return parser::Op::Or;
        else if (str == "!") return parser::Op::Not;
        else if (str == "==") return parser::Op::Eq;
        else if (str == "!=") return parser::Op::Neq;
        else if (str == "<") return parser::Op::Lst;
        else if (str == ">") return parser::Op::Grt;
        else if (str == "<=") return parser::Op::Lset;
        else if (str == ">=") return parser::Op::Gret;
        else if (str == "+") return parser::Op::Add;
        else if (str == "-") return parser::Op::Sub;
        else if (str == "*") return parser::Op::Mul;
        else if (str == "/") return parser::Op::Div;
        else if (str == "^") return parser::Op::Pow;
        else throw std::runtime_error("could not understand operator");
    }
};
...
BENCHMARK_F(CodegenDataFixture, GenerateBlank)(benchmark::State &state) {
    ...
    for (auto _ : state) {
        runtime::Proto proto = *codegen::generate_main_proto(std::move(_00_blank));
...
BENCHMARK_DEFINE_F(CodegenDataFixture, GenerateBasic)(benchmark::State &state) {
    ...
    for (auto _ : state) {
        runtime::Proto proto = *codegen::generate_main_proto(std::move(_01_basic));
        benchmark::DoNotOptimize(proto);
    }
};
